...
def non_max_suppression(
    ...
    """
    Taken from https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L162
...
class YoloWrapper(nn.Module):
    ...
    def forward(self, x):
        # Assuming x is a torch.Tensor of shape (B, H, W, C)
        scale = min(self.shape[0] / x.shape[1], self.shape[1] / x.shape[2])
        original_shape = int(x.shape[1]), int(x.shape[2])

        x = x.permute(0, 3, 1, 2)  # Change the shape of the image to BCHW
        # Compute scale factor
        x = F.interpolate(x, scale_factor=float(scale), mode='bilinear', align_corners=False)

        # Padding
        _, _, h, w = x.shape
        pad_h = (self.shape[0] - h) / 2
        pad_w = (self.shape[1] - w) / 2
        x = F.pad(x, (int(pad_w), int(pad_w), int(pad_h), int(pad_h)), value=114)
        ...
        for i, pred in enumerate(preds):
            pred[:, :4] = ops.scale_boxes(self.shape, pred[:, :4], original_shape)
        return preds
...
output_names = ['output']
dynamic_axes = {
    "images": {0: "batch", 1: "height", 2: "width"}
}
