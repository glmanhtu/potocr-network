...
from strhub.models.utils import get_pretrained_weights, LoggingCallback
...
def main(config: DictConfig):
    ...
    with open_dict(config):
        ...
        if gpu:
            ...
            if config.trainer.precision == 'bf16-mixed':
                if torch.get_autocast_gpu_dtype() is not torch.bfloat16:
                    raise ValueError('Mixed precision training with bfloat16 is not supported on this device')
    ...
    if config.pretrained is not None:
        if os.path.exists(config.pretrained):
            model.load_state_dict(torch.load(config.pretrained, map_location='cpu')['state_dict'])
        else:
        ...
        print(f'Loaded pretrained weights from {config.pretrained}')
    ...
    trainer: Trainer = hydra.utils.instantiate(
        ...
        callbacks=[checkpoint, swa, logging_callback],
    )
