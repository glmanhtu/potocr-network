...
class BaseSystem(pl.LightningModule, ABC):
    ...
    def _eval_step(self, batch, validation: bool) -> Optional[STEP_OUTPUT]:
        ...
        return dict(output=BatchResult(total, correct, ned, confidence, label_length, loss, loss_numel, lang_correct))
    ...
    def _aggregate_results(outputs: EPOCH_OUTPUT) -> tuple[float, float, float, float]:
        ...
        for result in outputs:
            ...
            total_lang_correct += result.lang_correct
        acc = total_n_correct / total_size
        ned = 1 - total_norm_ED / total_size
        loss = total_loss / total_loss_numel
        lang_acc = total_lang_correct / total_size
        return acc, ned, loss, lang_acc
    ...
    def on_validation_epoch_end(self) -> None:
        acc, ned, loss, lang_acc = self._aggregate_results(self.outputs)
        self.outputs.clear()
        self.log('val_accuracy', 100 * acc, sync_dist=True)
        self.log('val_NED', 100 * ned, sync_dist=True)
        self.log('val_loss', loss, sync_dist=True)
        self.log('val_lang_acc', 100 * lang_acc, sync_dist=True)
...
class CrossEntropySystem(BaseSystem):
    ...
    def __init__(
        self, tokenizer: str, batch_size: int, lr: float, warmup_pct: float, weight_decay: float, max_label_length: int
    ) -> None:
        tokenizer = BPEWrapper(tokenizer, max_label_length)
